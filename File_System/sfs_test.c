...
main(int argc, char **argv)
{
  ...
  /*int test=sfs_fopen("hello.txt");
  int test2=sfs_fopen("world.txt");
  sfs_fclose(test);
  sfs_fclose(test2);
  sfs_fclose(test);
  sfs_fclose(test2);
  test=sfs_fopen("hello.txt");
  test2=sfs_fopen("world.txt");
  
  sfs_fwrite(test,"hello world mother fucker!",27);
  sfs_remove("hello.txt");
  ...
  for (j = 0; j < strlen(test_str); j++) {
    for (i = 0; i < nopen; i++) {
      ...
      if (ch != test_str[j]) {
        fprintf(stderr, "ERROR: Read wrong byte from %s at %d (%d,%d) at fileID: %d\n", 
                names[i], j, ch, test_str[j],fds[i]);
        error_count++;
        break;
      }
    }
